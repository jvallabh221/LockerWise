...
const Feature = ({ Icon, title, body }) => (
    <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-6 shadow-none transition-shadow duration-1 ease-lw hover:shadow-sm">
        <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-lg bg-brass-50 text-brass-500 dark:bg-[var(--surface-2)]">
            <Icon className="h-5 w-5" strokeWidth={1.75} />
        </div>
        <h3 className="mb-1.5 font-display text-base font-semibold text-[var(--text)]">{title}</h3>
        <p className="text-sm leading-relaxed text-[var(--text-2)]">{body}</p>
    </div>
);

const Kpi = ({ label, value, hint }) => (
    <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-5 shadow-none">
        <div className="mb-2 text-xs font-medium uppercase tracking-wide text-[var(--text-3)]">{label}</div>
        <div className="font-display text-2xl font-semibold leading-none text-[var(--text)]">{value}</div>
        {hint ? <div className="mt-1.5 text-xs text-[var(--text-3)]">{hint}</div> : null}
    </div>
);

const Bullet = ({ children }) => (
    <li className="flex items-start gap-2.5 text-[var(--text-2)]">
        <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-success-500" strokeWidth={1.75} />
...
const Home = () => {
    return (
        <div className="lw-page flex min-h-screen flex-col">
            <DashNav />

            {/* Hero — marketing width cap §6 */}
            <section className="relative overflow-hidden px-6 pb-24 pt-16 sm:px-10 lg:px-16">
                <div
                    ...
                    style={{
                        backgroundImage:
                            "radial-gradient(circle at 85% 0%, color-mix(in srgb, var(--brand-500) 10%, transparent), transparent 50%), radial-gradient(circle at 0% 100%, color-mix(in srgb, var(--brand-300) 35%, transparent), transparent 45%)",
                    }}
                />
                <div className="relative mx-auto max-w-[1440px]">
                    ...
                    <h1 className="mx-auto mt-6 max-w-4xl text-center font-display text-4xl font-semibold leading-[1.05] tracking-tight text-[var(--text)] sm:text-5xl lg:text-6xl">
                        Locker operations, <span className="text-brass-500">streamlined.</span>
                    </h1>
                    <p className="mx-auto mt-5 max-w-2xl text-center text-lg leading-relaxed text-[var(--text-2)]">
                    ...
                    <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
                        ...
                        <Link
                            to="/contact-us"
                            className="inline-flex items-center gap-2 rounded-md border border-[var(--border-strong)] bg-[var(--surface)] px-5 py-2.5 text-sm font-medium text-[var(--text)] transition-colors hover:bg-[var(--bg)]"
                        >
                            Talk to us
                        </Link>
                    </div>
            ...
            {/* Features */}
            <section className="border-t border-[var(--border)] bg-[var(--surface)] px-6 py-20 sm:px-10 lg:px-16">
                <div className="mx-auto max-w-[1440px]">
                    <div className="mb-12 max-w-2xl">
                        <div className="lw-eyebrow mb-2">What it does</div>
                        <h2 className="font-display text-3xl font-semibold leading-tight text-[var(--text)] sm:text-4xl">
            ...
            {/* Why LockerWise */}
            <section className="border-t border-[var(--border)] px-6 py-20 sm:px-10 lg:px-16">
                <div className="mx-auto grid max-w-[1440px] grid-cols-1 items-start gap-12 lg:grid-cols-12">
                    ...
                    <div className="lg:col-span-7 grid sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <h3 className="mb-1.5 font-display text-base font-semibold text-[var(--text)]">Operationally strong</h3>
                        ...
                        <div>
                            ...
                            <h3 className="mb-1.5 font-display text-base font-semibold text-[var(--text)]">Secure by default</h3>
                            <ul className="space-y-2">
            ...
            {/* CTA */}
            <section className="border-t border-[var(--border)] bg-[var(--bg)] px-6 py-16 sm:px-10 lg:px-16">
                <div className="relative mx-auto max-w-[1440px] overflow-hidden rounded-2xl bg-ink-900 p-8 text-white sm:p-12">
                    <div
                        aria-hidden
                        className="absolute inset-0 opacity-[0.12] pointer-events-none"
                        style={{
                            backgroundImage:
                                "radial-gradient(circle at 90% 20%, var(--brand-500) 0, transparent 40%)",
                        }}
                    />
            ...
            {/* Footer */}
            <footer className="mt-auto bg-ink-900 px-6 py-12 text-white/80 sm:px-10 lg:px-16">
